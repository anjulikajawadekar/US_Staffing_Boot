...
//                    " status_id= "+z1.getStatus_id());
	...
	/*
	 * public ResponseEntity<?> AddRequisition2(String requisition_from, Integer id,
	 * String client, String job_title, String duration, String client_rate, String
	 * location, String position_type, String skills, int recruiter_id) {
	 * 
	 ...
	 * crt.add(Restrictions.eq("id", id));
	 * 
	 * Requisition z = (Requisition) crt.uniqueResult(); System.out.print("z = " +
	 * z.getRequisition_id());
	 * 
	 * if (z != null) { int a = z.getRequisition_id(); Criteria crt1 =
	 * session.createCriteria(StatusTbl.class);
	 * crt1.add(Restrictions.eq("recruiter.recruiter_id", recruiter_id));
	 * crt1.add(Restrictions.eq("status", "Assigned"));
	 * crt1.add(Restrictions.eq("requisition.requisition_id", a));
	 * System.out.println(a); StatusTbl z1 = (StatusTbl) crt1.uniqueResult();
	 * 
	 * if (z1 != null) { System.out.println("record exists"); return new
	 * ResponseEntity<StatusTbl>(z1, HttpStatus.OK); }
	 * 
	 * System.out.println("exist"); statusTbl.setStatus("Assigned");
	 * statusTbl.setFlag(true); statusTbl.setStatus_date(now);
	 * recruiter.setRecruiter_id(recruiter_id); statusTbl.setRecruiter(recruiter);
	 * statusTbl.setRequisition(requisition);
	 * 
	 * session.save(statusTbl);
	 * 
	 ...
	 * requisition.setRequisition_from(requisition_from); requisition.setId(id);
	 * requisition.setClient(client); requisition.setJob_title(job_title);
	 * requisition.setDuration(duration); requisition.setClient_rate(client_rate);
	 * requisition.setLocation(location);
	 * requisition.setPosition_type(position_type); requisition.setSkills(skills);
	 * 
	 * statusTbl.setStatus("Assigned"); statusTbl.setFlag(true);
	 * statusTbl.setStatus_date(now); recruiter.setRecruiter_id(recruiter_id);
	 * statusTbl.setRecruiter(recruiter); statusTbl.setRequisition(requisition);
	 * statusTbl.setFlag(true);
	 * 
	 * session.save(requisition); session.save(statusTbl);
	 * 
	 * transaction.commit(); session.close();
	 * 
	 * return new ResponseEntity<Requisition>(requisition, HttpStatus.OK); } }
